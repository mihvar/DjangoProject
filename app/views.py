...
import razorpay
from . models import OrderPlaced, Product, Wishlist
...
class CustomerRegistrationView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            form.save()
            messages.success(request,"Congrts! User Register Successfully")
        else:
...
#receipt_id = str(uuid.uuid4())[:40]  # Generate a unique receipt ID using uuid
@method_decorator(login_required, name='dispatch')
class CheckoutView(View):
    def get(self,request):
        ...
        if order_status == 'created':
             ...
             payment.save()
        return render(request, 'app/checkoutView.html',locals())
...
def contact_view(request):
    if request.method == "POST":
        ...
        contact = Contact(name=name, email=email, address=address, city=city, state=state, zip=zip_code, phone=phone)
        contact.save()
